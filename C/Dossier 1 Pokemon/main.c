...
void rechercheTypeEspece(struct espece especes[],struct indEspece index[],int nEspece)
{
    ...
    for (i=0; i<nEspece; i++)
    {
        if(strcmp(index[i].type, types[choixType-1])==0)
        {
            * pVec=index[i].posI;
            pVec ++;
            max++;
            printf("\nNom: %s\n",especes[index[i].posI].nomEspece);
            printf("Bonbons: %d\n",especes[index[i].posI].bonbons);
            printf("Points de vie: %u\n",especes[index[i].posI].pvMax);
            printf("Points de combats: %u\n",especes[index[i].posI].pcMax);
        }
